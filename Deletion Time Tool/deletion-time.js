...
function generateDeletionLink1() {
    ...
    html +=
        "<blockquote><strong>[[iframe https://arandintday.github.io/thebackhubscn/Deletion%20Time%20Tool/Deletion%20Time%20Tool.html?timestamp=" +
        timestamp.getTime() +
        "&type=" +
        type +
        custom1 +
        custom2 +
        ' style="width: 400px; height: 60px;"]]</strong></blockquote>';
    html += "</a";
    html += "<hr></hr>";
    html += '<a href="javascript:;" onclick="copy(1,2)" class="timer">';
    html += "<blockquote><strong>由于条目的分数为-X分，现根据[[[deletions-policy|删除政策]]]，宣告将删除此页：";
    html += "<br>";
    html +=
        "[[iframe https://arandintday.github.io/thebackhubscn/Deletion%20Time%20Tool/Deletion%20Time%20Tool.html?timestamp=" +
        timestamp.getTime() +
        "&type=" +
        type +
        custom1 +
    ...
    html += "<br>";
    html +=
        "如果你不是作者又想要重写该条目，请在此帖回复申请。请先取得作者的同意，并将原文的源代码复制至沙盒里。除非你是工作人员，否则请勿就申请重写以外的范围回复此帖。</strong></blockquote>";
    html += "</a>";
    html += "<hr></hr>";
    ...
    html += "<div>";
    html +=
        '<iframe src="https://arandintday.github.io/thebackhubscn/Deletion%20Time%20Tool/Deletion%20Time%20Tool.html?timestamp=' +
        timestamp.getTime() +
        "&type=" +
        type +
        custom1 +
        custom2 +
        '" style="width: 400px; height: 60px;"></iframe>';
    html += "</div>";
    $("#generated").html(html);
}
    ...
    switch (linkType) {
        ...
        case 2:
            var timestamp = new Date(
                now.getTime() + day * 24 * 60 * 60 * 1000 + hour * 60 * 60 * 1000 + minute * 60 * 1000
            );
            break;
    }
    return timestamp;
...
function initGenerators() {
    ...
    for (i = 0; i < 31; i++) {
        if (i == 3) html += '<option value="' + i + '" selected="selected">' + i + "</option>";
